...
const checkUserAndPass = async (username, password, done) => {
    ...
    if (result.length) {
        ...
        if (verifyPassword(user, password)) {
            ...
            console.log(`Password incorect for user ${username}`);
        }
    } else {
        console.log(`No user found with username ${username}`);
    }
    return done (null, false); // username or password were incorect 
}

const startegy = new BasicStrategy(checkUserAndPass);
